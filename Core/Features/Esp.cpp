...
void Core::Features::cEsp::Draw( )
{
	...
	for ( auto & Entity : Core::SDK::Game::EntityList )
	{
		...
		if (Cfg->FriendsMarker && !IsLocalPlayer)
		{
			...
			std::string PlayerSetFriend = xorstr("Inimigo");
			std::string PlayerUnFriend = xorstr("Friend");
			...
			if (IsFriend) 
			{
				...
				ImVec2 textSize = g_Variables.m_FontSmaller->CalcTextSizeA(
					g_Variables.m_FontSmaller->FontSize, FLT_MAX, 0.0f, PlayerUnFriend.c_str());
				DrawList->AddText(ImVec2(BoxCenter.x - (textSize.x / 2), BoxCenter.y - 15), ImColor(0, 0, 0), PlayerUnFriend.c_str());
				DrawList->AddText(ImVec2(BoxCenter.x - (textSize.x / 2), BoxCenter.y - 16), ImColor(g_Col.Base), PlayerUnFriend.c_str());
			...
			else if (FovSize < FovRadius) 
			{
				DrawList->AddCircleFilled(ImVec2(BoxCenter.x, BoxCenter.y), radius + Distance < 1 ? 1 : 4, ImColor(0, 0, 0), 999);
				DrawList->AddCircleFilled(ImVec2(BoxCenter.x, BoxCenter.y), radius + Distance < 1 ? 0 : 3, ImColor(255, 0, 0), 999);			
		...
		if (Cfg->Skeleton)
		{
			...
			float stick = static_cast<float>(g_Config.ESP->SkeletonThickness);

			DrawList->AddLine(ImVec2(ScreenHead.x, ScreenHead.y), ImVec2(Neck.x, Neck.y), Cfg->SkeletonCol, stick);
			DrawList->AddLine(ImVec2(LeftUperarm.x, LeftUperarm.y), ImVec2(RightUperarm.x, RightUperarm.y), Cfg->SkeletonCol, stick);
			DrawList->AddLine(ImVec2(RightUperarm.x, RightUperarm.y), ImVec2(RightFormArm.x, RightFormArm.y), Cfg->SkeletonCol, stick);
			DrawList->AddLine(ImVec2(LeftUperarm.x, LeftUperarm.y), ImVec2(LeftFormArm.x, LeftFormArm.y), Cfg->SkeletonCol, stick);
			DrawList->AddLine(ImVec2(RightFormArm.x, RightFormArm.y), ImVec2(RightHand.x, RightHand.y), Cfg->SkeletonCol, stick);
			DrawList->AddLine(ImVec2(LeftFormArm.x, LeftFormArm.y), ImVec2(LeftHand.x, LeftHand.y), Cfg->SkeletonCol, stick);
			DrawList->AddLine(ImVec2(Neck.x, Neck.y), ImVec2(Pelvis.x, Pelvis.y), Cfg->SkeletonCol, stick);
			DrawList->AddLine(ImVec2(Pelvis.x, Pelvis.y), ImVec2(LeftThigh.x, LeftThigh.y), Cfg->SkeletonCol, stick);
			DrawList->AddLine(ImVec2(Pelvis.x, Pelvis.y), ImVec2(RightThigh.x, RightThigh.y), Cfg->SkeletonCol, stick);
			DrawList->AddLine(ImVec2(LeftThigh.x, LeftThigh.y), ImVec2(LeftCalf.x, LeftCalf.y), Cfg->SkeletonCol, stick);
			DrawList->AddLine(ImVec2(RightThigh.x, RightThigh.y), ImVec2(RightCalf.x, RightCalf.y), Cfg->SkeletonCol, stick);
			DrawList->AddLine(ImVec2(LeftCalf.x, LeftCalf.y), ImVec2(Lfoot.x, Lfoot.y), Cfg->SkeletonCol, stick);
			DrawList->AddLine(ImVec2(RightCalf.x, RightCalf.y), ImVec2(Rfoot.x, Rfoot.y), Cfg->SkeletonCol, stick);
		}
...
void Core::Features::cEsp::DrawVehicle( )
{
	...
	for ( auto Entity : Core::SDK::Game::VehicleList )
	{
		...
		if ( Cfg->ShowLockUnlock ) {
			...
			const static int fovRadius = 100;
