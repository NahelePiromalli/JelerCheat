...
#include <Includes/ImGui/Files/menu_settings.hpp>
#include "imgui.h"
...
bool ImGui::ButtonEx( const char * label, const ImVec2 & size_arg, ImGuiButtonFlags flags )
{
	...
	if ( size.x <= 0.0f )
		size.x = GetWindowContentRegionMax( ).x - GetWindowContentRegionMin( ).x - ( window->ScrollbarY ? 5.f : 0.f ); // Arbitrary minimum child size (0.0f causing too many issues)
	if ( size.y <= 0.0f )
		size.y = 28.f;
	...
	if ( it_anim->second.alpha >= 0.99f )
		it_anim->second.active = false;

	it_anim->second.label_color = ImLerp( it_anim->second.label_color, it_anim->second.active ? colors::button_label_active : colors::button_label_inactive, g.IO.DeltaTime * 12.f );
	it_anim->second.alpha = ImClamp( it_anim->second.alpha + ( 6.f * g.IO.DeltaTime * ( it_anim->second.active ? 1.f : -1.f ) ), 0.f, 1.f );

	window->DrawList->AddRectFilled( rect.Min, rect.Max, GetColorU32( colors::button_background ), 4.f );
	window->DrawList->AddRect( rect.Min, rect.Max, GetColorU32( colors::button_outline ), 4.f );
	window->DrawList->AddRectFilled( rect.Min, rect.Max, GetColorU32( colors::accent, 0.1f * it_anim->second.alpha ), 4.f );
	window->DrawList->AddRect( rect.Min, rect.Max, GetColorU32( colors::accent, it_anim->second.alpha ), 4.f );

	PushFont( g_Variables.m_FontNormal );
	PushStyleColor( ImGuiCol_Text, it_anim->second.label_color );
	RenderTextClipped( rect.Min - ImVec2( 0, 2 ), rect.Max - ImVec2( 0, 2 ), label, NULL, NULL, ImVec2( 0.5f, 0.5f ) );
	PopStyleColor( );
	PopFont( );
...
bool ImGui::ScrollbarEx( const ImRect & bb_frame, ImGuiID id, ImGuiAxis axis, ImS64 * p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags )
{
	...
	ImRect bb = ImRect( bb_frame.Min - ImVec2( 5, -5 ), bb_frame.Max - ImVec2( 5, 5 ) );
	...
	// Render
	//window->DrawList->AddRectFilled( bb.Min, bb.Max, GetColorU32( window->Flags & ImGuiWindowFlags_ChildWindow ? colors::scrollbar_background : colors::scrollbar_background_two ), 8.f );
	ImRect grab_rect;
	if ( axis == ImGuiAxis_X )
	...
	window->DrawList->AddRectFilled( grab_rect.Min, grab_rect.Max, GetColorU32( g_Col.Base ), 8.f );
	return held;
}
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
	...
	// Verifica se está em estado de hover
	bool hovered = IsMouseHoveringRect(bb.Min, bb.Max);
	if (hovered)
	{
		// Adiciona um efeito de blur na imagem
		window->DrawList->AddImageRounded(user_texture_id, bb.Min, bb.Max, uv0, uv1, GetColorU32(tint_col), 10.0f);
		// Aqui você pode aplicar um efeito de blur na imagem usando técnicas específicas do ImGui, como shaders, se estiver implementando shaders personalizados
	}
	else
	{
		// Adiciona borda arredondada normalmente
		if (border_col.w > 0.0f)
		{
			window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(border_col), 10.0f);
			window->DrawList->AddImageRounded(user_texture_id, bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), uv0, uv1, GetColorU32(tint_col), 10.0f);
		}
		else
		{
			window->DrawList->AddImageRounded(user_texture_id, bb.Min, bb.Max, uv0, uv1, GetColorU32(tint_col), 10.0f);
...
struct checkbox_state {
	...
	ImVec4 background = colors::checkbox_background;
	ImVec4 border = colors::checkbox_outline;
	ImVec2 offset, pump;
	float alpha = 0.f, alpha_mark = 0.f, up_check = 0.f, mark_size;
...
bool ImGui::Checkbox( const char * label, bool * v )
{
	...
	const float width = GetWindowContentRegionMax( ).x - GetWindowContentRegionMin( ).x - ( window->ScrollbarY ? 5.f : 0.f );
	const ImVec2 pos = window->DC.CursorPos;
	const ImRect rect( pos, pos + ImVec2( width, 10 ) );
	const ImRect clickable( pos - ImVec2( 0, 4 ), pos + ImVec2( width, 14 ) );
	ItemSize( rect, style.FramePadding.y );
	if ( !ItemAdd( rect, id, &clickable ) )
	{
	...
	it_anim->second.label_color = ImLerp( it_anim->second.label_color, *v ? colors::checkbox_label_active : colors::checkbox_label_inactive, g.IO.DeltaTime * 8.f );
	it_anim->second.background = ImLerp( it_anim->second.background, *v ? colors::accent : colors::checkbox_background, g.IO.DeltaTime * 12.f );
	it_anim->second.offset = ImLerp( it_anim->second.offset, *v ? it_anim->second.pump : ImVec2( 18, 18 ) / 2, g.IO.DeltaTime * 12.f );
	it_anim->second.pump = ImLerp( it_anim->second.pump, it_anim->second.alpha >= 0.99f ? ImVec2( 0, 0 ) : -ImVec2( 2, 2 ), g.IO.DeltaTime * 16.f );
	it_anim->second.alpha_mark = ImLerp( it_anim->second.alpha_mark, *v ? 1.f : 0.f, g.IO.DeltaTime * 8.f );
	it_anim->second.alpha = ImLerp( it_anim->second.alpha, *v ? 1.f : 0.f, g.IO.DeltaTime * 10.f );
	it_anim->second.up_check = ImLerp( it_anim->second.up_check, *v ? 18.f : 0.f, g.IO.DeltaTime * 8.f );

	window->DrawList->AddText( g_Variables.m_FontNormal, g_Variables.m_FontNormal->FontSize, rect.Min - ImVec2( 1, 4 ), GetColorU32( it_anim->second.label_color ), label );

	window->DrawList->AddRectFilled( ImVec2( clickable.Max.x - 18, clickable.Min.y ), clickable.Max, GetColorU32( colors::checkbox_background ), 4.f );
	window->DrawList->AddRect( ImVec2( clickable.Max.x - 18, clickable.Min.y ), clickable.Max, GetColorU32( colors::checkbox_outline ), 4.f );
	window->DrawList->AddRectFilled( ImVec2( clickable.Max.x - 18, clickable.Min.y ) + it_anim->second.offset, clickable.Max - it_anim->second.offset, GetColorU32( it_anim->second.background ), 4.f );
	RenderCheckMark( window->DrawList, ImVec2( clickable.Max.x - 18, clickable.Max.y - it_anim->second.up_check ) + ( ImVec2( 18, 18 ) / 2 ) / 2, GetColorU32( ImVec4( 12.f / 255.f, 12.f / 255.f, 12.f / 255.f, it_anim->second.alpha_mark ) ), 18.f / 2.f );
...
void ImGui::SeparatorEx( ImGuiSeparatorFlags flags, float thickness )
{
	...
	if ( flags & ImGuiSeparatorFlags_Vertical )
	{
		...
		// Draw
		window->DrawList->AddRectFilled( bb.Min, bb.Max, GetColorU32( g_Col.ChildBorderCol ) );
		if ( g.LogEnabled )
			LogText( " |" );
	}
...
struct combo_state {
	...
	bool hovered = false;
	float alpha = 0.f;
	float expand_rotate = 0.f;
	ImVec4 expand_color = colors::combo_arrow;

	float Down = 0.f;
};

bool ImGui::BeginCombo( const char * label, const char * preview_value, int val, ImGuiComboFlags flags, bool multi )
{
	ImGuiContext & g = *GImGui;
	ImGuiWindow * window = GetCurrentWindow( );
	...
	const ImRect rect( pos, pos + ImVec2( width, 51 ) );
	const ImRect clickable( pos + ImVec2( 0, 23 ), pos + ImVec2( width, 51 ) );
	...
	if ( hovered && g.IO.MouseClicked[ 0 ] || it_anim->second.combo_opened && g.IO.MouseClicked[ 0 ] && !it_anim->second.hovered )
		it_anim->second.combo_opened = !it_anim->second.combo_opened;

	it_anim->second.alpha = ImClamp( it_anim->second.alpha + ( 8.f * g.IO.DeltaTime * ( it_anim->second.combo_opened ? 1.f : -1.f ) ), 0.f, 1.f );
	it_anim->second.expand_color = ImLerp( it_anim->second.expand_color, it_anim->second.combo_opened ? g_Col.Base : g_Col.Base, g.IO.DeltaTime * 10.f );
	it_anim->second.expand_rotate = ImLerp( it_anim->second.expand_rotate, it_anim->second.combo_opened ? 1.f : -1.f, g.IO.DeltaTime * 10.f );

	if ( !IsRectVisible( clickable.Min, clickable.Max + ImVec2( 0, 2 ) ) )
	{
		it_anim->second.combo_opened = false;
		it_anim->second.alpha = 0.f;
	}

	//if ( strchr( label, '#' ) == NULL ) {
	window->DrawList->AddText( g_Variables.m_FontNormal, g_Variables.m_FontNormal->FontSize, rect.Min - ImVec2( 1, 4 ), GetColorU32( g_Col.SecundaryFeaturesText ), label );
	//}
	window->DrawList->AddRectFilled( clickable.Min, clickable.Max, GetColorU32( ImVec4( ImColor( 18, 18, 20 ) ) ), 4.f );
	window->DrawList->AddRect( clickable.Min, clickable.Max, GetColorU32( ImVec4( ImColor( 23, 23, 25 ) ) ), 4.f );

	ImRotateStart( );
	window->DrawList->AddText( g_Variables.m_Expand, g_Variables.m_Expand->FontSize, rect.Max - ImVec2( 21, 19 ), GetColorU32( it_anim->second.expand_color ), "A" );
	ImRotateEnd( 1.57f * it_anim->second.expand_rotate );

	PushFont( g_Variables.m_FontNormal );
	PushStyleColor( ImGuiCol_Text, GetColorU32( g_Col.FeaturesText ) );
	RenderTextClipped( clickable.Min - ImVec2( -8, 2 ), clickable.Max - ImVec2( 32, 2 ), preview_value, NULL, NULL, ImVec2( 0.f, 0.5f ) );
	PopStyleColor( );
	PopFont( );

	window->DrawList->AddRectFilledMultiColor( ImVec2( clickable.Max.x - 45, clickable.Min.y + 6 ), clickable.Max - ImVec2( 32, 6 ), GetColorU32( ImVec4( ImColor( 18, 18, 20 ) ), 0.f ), GetColorU32( ImVec4( ImColor( 18, 18, 20 ) ) ), GetColorU32( ImVec4( ImColor( 18, 18, 20 ) ) ), GetColorU32( ImVec4( ImColor( 18, 18, 20 ) ), 0.f ) );

	if ( !it_anim->second.combo_opened && it_anim->second.alpha < 0.1f )
		return false;

	ImGuiWindowFlags window_flags = ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_WithoutScrollClamp;

	PushStyleColor( ImGuiCol_WindowBg, GetColorU32( ImVec4( ImColor( 16, 16, 18 ) ) ) );
	PushStyleColor( ImGuiCol_Border, GetColorU32( ImVec4( ImColor( 18, 18, 20 ) ) ) );
	...
	it_anim->second.Down = ImLerp( it_anim->second.Down, it_anim->second.combo_opened ? ( g.Style.WindowPadding.y * 2 ) + ( 22 * val ) + ( g.Style.ItemSpacing.y * ( val - 1 ) ) : 0.f, ImGui::GetIO( ).DeltaTime * 6 );

	SetNextWindowPos( clickable.GetBL( ) + ImVec2( 0, 5 ) );
	SetNextWindowSize( ImVec2( clickable.GetWidth( ), it_anim->second.Down ) );
...
bool ImGui::Combo( const char * label, int * current_item, const char * ( *getter )( void * user_data, int idx ), void * user_data, int items_count, int popup_max_height_in_items )
{
	...
	for ( int i = 0; i < items_count; i++ )
	{
		...
		if ( CustomSelectableEx( item_text, item_selected ) && *current_item != i )
		{
			value_changed = true;
			*current_item = i;
		}
...
//
//bool Backup( const char * label, ImGuiDataType data_type, void * p_data, const void * p_min, const void * p_max, const char * format, ImGuiSliderFlags flags )
//{
//	ImGuiWindow * window = GetCurrentWindow( );
//	if ( window->SkipItems )
//		return false;
...
//	const char * value_buf_end = value_buf + DataTypeFormatString( value_buf, IM_ARRAYSIZE( value_buf ), data_type, p_data, format );
//
//	window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, ImGui::GetColorU32( ( ImVec4 ) ImColor( 32, 33, 35 ) ), 5.0f );
//	window->DrawList->AddRectFilled( frame_bb.Min, ImVec2( grab_bb.Min.x + 3, frame_bb.Max.y ), ImGui::GetColorU32( ( ImVec4 ) ImColor( 22, 118, 243 ) ), 5.0f );
//
//	auto TextSize = ImGui::CalcTextSize( label );
//	PushStyleColor( ImGuiCol_Text, ImVec4( g_Col.FeaturesText ) ); {
//		RenderText( total_bb.Min, label );
//	}
//
//	PushStyleColor( ImGuiCol_Text, ImVec4( g_Col.SecundaryText ) ); {
//		RenderTextClipped( ImVec2( total_bb.Min.x + TextSize.x + 10, total_bb.Min.y ), total_bb.Max, value_buf, value_buf_end, NULL );
//	} PopStyleColor( );
//
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	const ImVec2 Pos = Window->DC.CursorPos;
	const float SliderY = 15;
	const float Width = ImGui::GetWindowContentRegionMax().x - ImGui::GetWindowContentRegionMin().x - (Window->ScrollbarY ? 5.f : 0.f);

	const ImVec2 LabelSize = CalcTextSize(label, NULL, true);
	const ImRect Rect(Pos, Pos + ImVec2(Width, LabelSize.y + SliderY));
	const ImRect SliderRect(Rect.Min + ImVec2(0, LabelSize.y + 4), Rect.Max);

	ItemSize(Rect, style.FramePadding.y);
	if (!ItemAdd(Rect, id, &SliderRect))
		return false;
	...
	struct Slider_t {
		ImVec4 BackGroundCol = ImColor(18, 18, 18);
		float Slow;
	};

	static std::map<ImGuiID, Slider_t> anim;
	auto SliderAnim = anim.find(id);
	if (SliderAnim == anim.end())
	{
		anim.insert({ id, Slider_t() });
		SliderAnim = anim.find(id);
	}

	ImRect GrabRect;
	const bool ValueChanged = SliderBehavior(SliderRect, id, data_type, p_data, p_min, p_max, format, flags, &GrabRect);
	if (ValueChanged) {
		MarkItemEdited(id);
	}
	...
	SliderAnim->second.Slow = ImLerp(SliderAnim->second.Slow, GrabRect.Min.x - SliderRect.Min.x + 13, g.IO.DeltaTime * 10.f);
	SliderAnim->second.BackGroundCol = ImLerp(SliderAnim->second.BackGroundCol, Hovered ? ImColor(20, 20, 20) : ImColor(18, 18, 18), g.IO.DeltaTime * 6.f);


	float filledWidth = SliderAnim->second.Slow;
	float barHeight = 8.0f; 

	
	float barY = SliderRect.Min.y + (SliderRect.GetHeight() / 2) - (barHeight / 2);

	
	Window->DrawList->AddRectFilled(SliderRect.Min, SliderRect.Max, ImGui::GetColorU32(SliderAnim->second.BackGroundCol), 5);

	
	Window->DrawList->AddRect(SliderRect.Min, SliderRect.Max, barHeight, ImGui::GetColorU32(ImVec4(ImColor(20, 20, 20))), 5);

	
	Window->DrawList->AddRectFilled(
		ImVec2(SliderRect.Min.x, barY),
		ImVec2(SliderRect.Min.x + filledWidth, barY + barHeight),
		ImGui::GetColorU32(g_Col.Base),
		5
	);
	...
	auto TextSize = ImGui::CalcTextSize(label);
	Window->DrawList->AddText(Rect.Min, ImGui::GetColorU32(ImVec4(g_Col.FeaturesText)), label);

	ImGui::PushFont(g_Variables.m_FontSecundary);
	auto ValueSize = ImGui::CalcTextSize(value_buf);
	Window->DrawList->AddText(ImVec2((Pos.x + Width) - ValueSize.x, Rect.Min.y), ImGui::GetColorU32(ImVec4(g_Col.SecundaryText)), value_buf);
	ImGui::PopFont();
	IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.ItemFlags);
	return ValueChanged;
}
...
bool ImGui::InputTextEx( const char * label, const char * hint, char * buf, int buf_size, const ImVec2 & size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void * callback_user_data )
{
	...
	float WindowAlpha = ImGui::GetStyle( ).Alpha;

	const ImRect frame_bb( window->DC.CursorPos + ImVec2( 0, 0 ), window->DC.CursorPos + frame_size + ImVec2( 0, 0 ) );
	...
	if ( !is_multiline )
	{
		window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, ImGui::GetColorU32( g_Col.InputBackground ), 4.f );
		window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, ImGui::GetColorU32( g_Col.InputBorder ), 4.f );
	}

	const ImVec4 clip_rect( frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y ); // Not using frame_bb.Max because we have adjusted size
	ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2( 10, 7 );
	...
	if ( render_cursor || render_selection )
	{
		...
		if ( render_selection )
		{
			...
			ImU32 bg_color = ImColor( 87, 83, 78, 75 ); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
			...
			for ( const ImWchar * p = text_selected_begin; p < text_selected_end; )
			{
				...
				else
				{
					...
					//float alpha;
					//alpha = ImLerp( alpha, rect.Overlaps( clip_rect ) ? 0.4f : 0.f, ImGui::GetIO( ).DeltaTime * 2.f );

					//ImVec4 BaseWithAlpha = ImVec4( g_Col.Base.x, g_Col.Base.y, g_Col.Base.z, alpha );

					//GetWindowDrawList( )->AddRectFilled( ImVec2( rect.Min.x - 2, rect.Min.y - 1 ), ImVec2( rect.Max.x + 2, rect.Max.y + 2 ), ImColor( BaseWithAlpha ), 4.0f );
				}
		...
		{
			ImU32 col = GetColorU32( ImVec4( ImColor( 200, 200, 200, 240 ) ) );
			draw_window->DrawList->AddText( g.Font, g.FontSize, draw_pos + ImVec2( 0, 0 ), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect );
		}
		...
		if ( render_cursor )
		{
			...
			it_anim->second.slow = ImLerp( it_anim->second.slow, cursor_screen_rect.Min.x - total_bb.Min.x, g.IO.DeltaTime * 15.f );


			if ( cursor_screen_rect.Overlaps( clip_rect ) ) {

				//float sdfsdfsdfsdd = ImTricks::Animations::FastLerpFloat( "cursorinput.anim", cursor_is_visible, 0, is_readonly == false ? 1 : 0, 99 );
				/// ImVec4 BaseWithAlpha = ImVec4( g_Col.Base.x, g_Col.Base.y, g_Col.Base.z, sdfsdfsdfsdd );
				//window->DrawList->AddLine( total_bb.Min + ImVec2( it_anim->second.slow, 8 ), total_bb.GetBL( ) - ImVec2( -it_anim->second.slow, 8 ), ImColor( BaseWithAlpha ) );
			}
...
bool ImGui::ColorEdit4( const char * label, float col[ 4 ], ImGuiColorEditFlags flags )
{
	...
	if ( it_anim == anim.end( ) )
	{
		anim.insert( { GetID( label ), color_state( ) } );
	...
	if ( !( flags & ImGuiColorEditFlags_NoSmallPreview ) )
	{
		...
		if ( ItemHoverable( g.LastItemData.Rect, g.LastItemData.ID, 0 ) && g.IO.MouseClicked[ 0 ] || it_anim->second.active && g.IO.MouseClicked[ 0 ] && !it_anim->second.hovered )
...
bool ImGui::ColorPicker4( const char * label, float col[ 4 ], ImGuiColorEditFlags flags, const float * ref_col )
{
	...
	float picker_width = 160.f;
	float picker_height = 115.f;
	float bars_width = picker_width;
	float bars_height = 12.f;
	float bars_pos_x = picker_pos.x;
	float hue_bar_pos_y = picker_pos.y + picker_height + 8;
	float alpha_bar_pos_y = picker_pos.y + picker_height + 28;
	...
	{
		// Render SV Square
		draw_list->AddRectFilledMultiColor( picker_pos, picker_pos + ImVec2( picker_width, picker_height - 1 ), col_white, hue_color32, hue_color32, col_white, 4.f );
		draw_list->AddRectFilledMultiColor( picker_pos, picker_pos + ImVec2( picker_width, picker_height ), 0, 0, col_black, col_black, 4.f );
		RenderFrameBorder( picker_pos, picker_pos + ImVec2( picker_width, picker_height ), 0.0f );
		sv_cursor_pos.x = ImLerp( sv_cursor_pos.x, ImClamp( IM_ROUND( picker_pos.x + ImSaturate( S ) * picker_width ), picker_pos.x + 2, picker_pos.x + picker_width - 2 ), ImGui::GetIO( ).DeltaTime * 8 ); // Sneakily prevent the circle to stick out too much
		sv_cursor_pos.y = ImLerp( sv_cursor_pos.y, ImClamp( IM_ROUND( picker_pos.y + ImSaturate( 1 - V ) * picker_height ), picker_pos.y + 2, picker_pos.y + picker_height - 2 ), ImGui::GetIO( ).DeltaTime * 8 );

		// Render Hue Bar
		for ( int i = 0; i < 6; ++i )
			draw_list->AddRectFilledMultiColor( ImVec2( bars_pos_x + i * ( bars_width / 6 ) - ( i == 1 || i == 5 ? 1 : 0 ), hue_bar_pos_y + 2 ), ImVec2( bars_pos_x + ( i + 1 ) * ( bars_width / 6 ), hue_bar_pos_y + ( bars_height - 2 ) ), col_hues[ i ], col_hues[ i + 1 ], col_hues[ i + 1 ], col_hues[ i ], i == 0 || i == 5 ? 4.f : 0.f, i == 0 ? ImDrawFlags_RoundCornersLeft : i == 5 ? ImDrawFlags_RoundCornersRight : ImDrawFlags_RoundCornersNone );
		static float bar0_line_x = 0.f;
		bar0_line_x = ImLerp( bar0_line_x, IM_ROUND( bars_pos_x + ( 1.f - H ) * bars_width ), ImGui::GetIO( ).DeltaTime * 8 );
		bar0_line_x = ImClamp( bar0_line_x, bars_pos_x + 3.f, bars_pos_x + bars_width - 3.f );
		draw_list->AddCircleFilled( ImVec2( bar0_line_x, hue_bar_pos_y + 6 ), 6, col_white, 30 );
		draw_list->AddCircleFilled( ImVec2( bar0_line_x, hue_bar_pos_y + 6 ), 2, GetColorU32( ImVec4( ImColor( 190, 190, 190 ) ) ), 30 );
	...
	if ( alpha_bar )
	{
		...
		draw_list->AddRectFilledMultiColor( ImVec2( bars_pos_x, alpha_bar_pos_y + 2 ), ImVec2( bars_pos_x + bars_width, alpha_bar_pos_y + bars_height - 2 ), col_black, hue_color32, hue_color32, col_black, 4.f );
		static float bar1_line_x = 0.f;
		bar1_line_x = ImLerp( bar1_line_x, IM_ROUND( bars_pos_x + alpha * bars_width ), ImGui::GetIO( ).DeltaTime * 8 );
		bar1_line_x = ImClamp( bar1_line_x, bars_pos_x + 3.f, bars_pos_x + bars_width - 3.f );
		draw_list->AddCircleFilled( ImVec2( bar1_line_x, alpha_bar_pos_y + 6 ), 6, col_white, 30 );
		draw_list->AddCircleFilled( ImVec2( bar1_line_x, alpha_bar_pos_y + 6 ), 2, GetColorU32( ImVec4( ImColor( 190, 190, 190 ) ) ), 30 );
	}
...
bool ImGui::ColorButton( const char * desc_id, const ImVec4 & col, ImGuiColorEditFlags flags, const ImVec2 & size_arg )
{
	...
	const float width = GetWindowContentRegionMax( ).x - GetWindowContentRegionMin( ).x - ( window->ScrollbarY ? 5.f : 0.f );
	const ImRect rect( window->DC.CursorPos, window->DC.CursorPos + ImVec2( width, 10 ) );
	const ImRect clickable( window->DC.CursorPos - ImVec2( 0, 4 ), window->DC.CursorPos + ImVec2( width, 14 ) );
	ItemSize( rect, ( size.y >= default_size ) ? g.Style.FramePadding.y : 0.0f );
	if ( !ItemAdd( rect, id ) )
		return false;
	...
	window->DrawList->AddText( g_Variables.m_FontNormal, g_Variables.m_FontNormal->FontSize, rect.Min - ImVec2( 1, 4 ), GetColorU32( g_Col.SecundaryFeaturesText ), desc_id );
	RenderColorRectWithAlphaCheckerboard( window->DrawList, ImVec2( clickable.Max.x - 18, clickable.Min.y ), clickable.Max, GetColorU32( col ), 9.f, ImVec2( 0.f, 0.f ), 8 );
...
struct tab_state {
	bool clicked = false;
	float size = 0.f;
	ImVec4 col = colors::tab_icon;
	ImVec4 col_selected = ImColor( 17, 17, 17 );
	ImVec4 col_selected_border = ImColor( 19, 19, 19 );
};
...
bool ImGui::Tab( const char * label, ImTextureID icon, bool active )
{
	...
	it_anim->second.col = ImLerp( it_anim->second.col, active ? colors::accent : colors::tab_icon, g.IO.DeltaTime * 8.f );

	if ( pressed && !active )
		it_anim->second.clicked = true;

	it_anim->second.size = ImLerp( it_anim->second.size, it_anim->second.clicked ? 2.f : 0.f, g.IO.DeltaTime * 16.f );

	if ( it_anim->second.size >= 1.9f )
		it_anim->second.clicked = false;

	it_anim->second.col_selected = ImLerp( it_anim->second.col_selected, active ? ImColor( 20, 20, 20, 255 ) : ImColor( 20, 20, 20, 0 ), g.IO.DeltaTime * 8.f );
	it_anim->second.col_selected_border = ImLerp( it_anim->second.col_selected_border, active ? ImColor( 21, 21, 21, 255 ) : ImColor( 21, 21, 21, 0 ), g.IO.DeltaTime * 8.f );

	window->DrawList->AddRectFilled( rect.Min, rect.Max, GetColorU32( it_anim->second.col_selected ), 6.0f, 0 );
	//window->DrawList->AddRect( rect.Min, rect.Max, GetColorU32( it_anim->second.col_selected_border ), 6.0f, 0 );

	window->DrawList->AddImage( icon, rect.Min + ImVec2( 12 + it_anim->second.size, 12 + it_anim->second.size ), rect.Max - ImVec2( 12 + it_anim->second.size, 12 + it_anim->second.size ), ImVec2( 0, 0 ), ImVec2( 1, 1 ), GetColorU32( it_anim->second.col ) );

	IMGUI_TEST_ENGINE_ITEM_INFO( id, label, g.LastItemData.StatusFlags );
	return pressed;
}

bool ImGui::CustomSelectableEx( const char * label, bool active )
{
	ImGuiWindow * window = GetCurrentWindow( );
	if ( window->SkipItems )
		return false;
	...
	const float width = GetWindowContentRegionMax( ).x - GetWindowContentRegionMin( ).x - ( window->ScrollbarY ? 5.f : 0.f );
	const ImVec2 pos = window->DC.CursorPos;
	const ImRect rect( pos, pos + ImVec2( width, 20 ) );
	ItemSize( rect, style.FramePadding.y );
	if ( !ItemAdd( rect, id ) )
		return false;

	const bool was_selected = active;
	...
	if ( it_anim == anim.end( ) )
	{
		anim.insert( { id, colors::checkbox_label_inactive } );
		it_anim = anim.find( id );
	}

	it_anim->second = ImLerp( it_anim->second, active ? g_Col.Base : colors::checkbox_label_inactive, g.IO.DeltaTime * 8.f );

	//window->DrawList->AddRect( rect.Min, rect.Max, GetColorU32( colors::selectable_outline ), 4.f );

	PushFont( g_Variables.m_FontSecundary );
	PushStyleColor( ImGuiCol_Text, GetColorU32( it_anim->second ) );
	RenderTextClipped( rect.Min - ImVec2( 0, 2 ), rect.Max - ImVec2( 0, 2 ), label, NULL, NULL, ImVec2( 0.5f, 0.5f ) );
	PopStyleColor( );
	PopFont( );
...
bool ImGui::CustomSelectable( const char * label, bool * p_selected )
{
	if ( CustomSelectableEx( label, *p_selected ) )
	{
		*p_selected = !*p_selected;
		return true;
	}
...
void ImGui::MultiCombo( const char * label, bool variable[ ], const char * labels[ ], int count )
{
	...
	if ( BeginCombo( label, preview.c_str( ), count, 0, true ) )
	{
		for ( auto i = 0; i < count; i++ )
			CustomSelectable( labels[ i ], &variable[ i ] );
		EndCombo( );
	}
...
bool ImGui::Keybind( const char * label, int * key, int * mode )
{
	...
	const ImVec2 pos = window->DC.CursorPos;
	const ImRect rect( pos, pos + ImVec2( width, 20 ) );
	const ImRect clickable( pos - ImVec2( 0, 0 ), pos + ImVec2( width, 20 ) );
	...
	window->DrawList->AddText( g_Variables.m_FontNormal, g_Variables.m_FontNormal->FontSize, rect.Min, GetColorU32( g_Col.FeaturesText ), label );

	auto KeyTxtSize = g_Variables.m_FontSecundary->CalcTextSizeA( g_Variables.m_FontSecundary->FontSize, FLT_MAX, 0.f, buf_display );

	it_anim->second.PosX = ImLerp( it_anim->second.PosX, KeyTxtSize.x + 6, g.IO.DeltaTime * 6 );
	it_anim->second.TextPosX = ImLerp( it_anim->second.TextPosX, KeyTxtSize.x, g.IO.DeltaTime * 6 );


	//window->DrawList->AddRect( ImVec2( rect.Max.x - it_anim->second.PosX, clickable.Min.y ), ImVec2( rect.Max.x + 6, clickable.Max.y ), GetColorU32( ImVec4( ImColor( 22, 22, 24 ) ) ), 4 );
	//window->DrawList->AddRect( ImVec2( pos.x + it_anim->second.PosX, clickable.Min.y ), ImVec2( rect.Max.x + 6, clickable.Max.y ), GetColorU32( ImVec4( ImColor( 24, 24, 26 ) ) ), 4 );

	window->DrawList->AddText( g_Variables.m_FontSecundary, g_Variables.m_FontSecundary->FontSize, ImVec2( clickable.Max.x - it_anim->second.TextPosX, rect.Min.y + 1 ), GetColorU32( g_Col.SecundaryText ), buf_display );
	...
	ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_WithoutScrollClamp;

	//if ( it_anim->second.alpha >= 0.01f )
	//{
	//	PushStyleColor( ImGuiCol_WindowBg, GetColorU32( colors::key_background ) );
	//	//PushStyleColor( ImGuiCol_Border, GetColorU32( colors::key_outline ) );
	//	PushStyleVar( ImGuiStyleVar_Alpha, it_anim->second.alpha );
	...
	//	SetNextWindowSize( ImVec2( 130, 104 ) );
	//	SetNextWindowPos( rect.GetBR( ) - ImVec2( 130, 13 ) );

	//	Begin( label, NULL, window_flags );
	//	{
	//		if ( CustomSelectableEx( "Hold", *mode == 0 ) )
	//		{
	//			*mode = 0;
	//			it_anim->second.active = false;
	//		}
	//		if ( CustomSelectableEx( "Toggle", *mode == 1 ) )
	//		{
	//			*mode = 1;
	//			it_anim->second.active = false;
	//		}
	//		if ( CustomSelectableEx( "Always", *mode == 2 ) )
	//		{
	//			*mode = 2;
	//			it_anim->second.active = false;
	//		}
...
bool ImGui::ListSelectableEx( const char * Label, bool Selected )
{
	ImGuiWindow * Window = GetCurrentWindow( );
	if ( Window->SkipItems )
		return false;
	...
	ImVec2 TextSize = Utils::CalcTextSize( g_Variables.m_FontNormal, g_Variables.m_FontNormal->FontSize, Label );
	const float Width = ImGui::GetWindowContentRegionMax( ).x - ImGui::GetWindowContentRegionMin( ).x - ( Window->ScrollbarY ? 5.f : 0.f );
	const float YSize = 28;
	const ImVec2 Pos = Window->DC.CursorPos;
	const ImRect Rect( Pos, Pos + ImVec2( Width, YSize ) );
	ItemSize( Rect, style.FramePadding.y );
	if ( !ItemAdd( Rect, id ) )
		return false;
	...
	struct Selectable_t {
		ImVec4 TextCol = ImColor( g_Col.SecundaryFeaturesText );
		ImVec4 BackGroundCol = ( ImVec4 ) ImColor( 18, 18, 20 );
		ImVec4 CircleCol = ImColor( );
		float Size = 0.f;
	};

	static std::map<ImGuiID, Selectable_t> anim;
	auto SelectableWidget = anim.find( id );

	if ( SelectableWidget == anim.end( ) ) {
		anim.insert( { id, Selectable_t( ) } );
		SelectableWidget = anim.find( id );
	}

	SelectableWidget->second.TextCol = ImLerp( SelectableWidget->second.TextCol, Selected || Hovered ? ImColor( g_Col.FeaturesText ) : ImColor( g_Col.SecundaryFeaturesText ), g.IO.DeltaTime * 8.f );
	SelectableWidget->second.BackGroundCol = ImLerp( SelectableWidget->second.BackGroundCol, Selected || Hovered ? ( ImVec4 ) ImColor( 20, 20, 22 ) : ( ImVec4 ) ImColor( 18, 18, 20 ), g.IO.DeltaTime * 8.f );
	SelectableWidget->second.CircleCol = ImLerp( SelectableWidget->second.CircleCol, Selected || Hovered ? ( ImVec4 ) ImColor( g_Col.Base ) : ImVec4( g_Col.Base.x, g_Col.Base.y, g_Col.Base.z, 0.f ), g.IO.DeltaTime * 16.f );
	SelectableWidget->second.Size = ImLerp( SelectableWidget->second.Size, Selected ? 4.f : 0.f, g.IO.DeltaTime * 8.f );

	Window->DrawList->AddRectFilled( Rect.Min, Rect.Max, GetColorU32( SelectableWidget->second.BackGroundCol ), 6 );
	Window->DrawList->AddRect( Rect.Min, Rect.Max, GetColorU32( ImVec4( ImColor( 24, 24, 26 ) ) ), 6 );

	ImVec2 TxtPos( Rect.Min.x + 10, Pos.y + ( YSize / 2 - TextSize.y / 2 ) );
	Window->DrawList->AddText( g_Variables.m_FontNormal, g_Variables.m_FontNormal->FontSize, ImVec2( TxtPos ), GetColorU32( SelectableWidget->second.TextCol ), Label );

	ImVec2 CirclePos( Rect.Max.x - 16, Pos.y + ( YSize ) / 2 );
	Window->DrawList->AddCircleFilled( CirclePos, SelectableWidget->second.Size, GetColorU32( ImVec4( SelectableWidget->second.CircleCol ) ), 99 );

	IMGUI_TEST_ENGINE_ITEM_INFO( id, Label, g.LastItemData.StatusFlags );
	return Pressed;
}

bool ImGui::ListSelectable( const char * label, bool * p_selected )
{
	if ( ListSelectableEx( label, *p_selected ) )
	{
		*p_selected = !*p_selected;
		return true;
	}
